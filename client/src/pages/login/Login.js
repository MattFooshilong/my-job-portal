...
const Login = () => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        password: Yup.string().required('Required').matches(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{4,}$/, 'Password must contain one digit, one lowercase, one uppercase and a special character'),
    })
    ...
    return (
        <Container className='py-3 pt-sm-5'>
            <Card className={`p-sm-5 p-4 mt-5 ${styles.card}`}>
                <h3>Hi!</h3>
                <h4>Welcome to My Job Portal</h4>
                <Formik
                    enableReinitialize
                ...
                >
                    {({ values, handleChange, errors, touched }) => (
                        <FormikForm>
                            ...
                            {err && <Alert variant='danger'>Wrong email or password</Alert>}
